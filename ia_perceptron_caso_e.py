...
if __name__ == "__main__":
    ...
    clasificacion = []
    CANT_DATOS = 30000
    # Creamos CANT_DATOS muestras mitad con error de posicion inestable y mitad deficiente instruccion
    for _ in tqdm(range(CANT_DATOS // 2)):
        tirador.tirar_mal_c_d_e(max_ancho=24, max_alto=6)
        clasificacion.append(1) # Lo clasificamos como 1 "Error posicion inestable"
    ...
    print(f'Acertó {clf.predict(matriz_prueba).sum()} de {CANT_PRUEBA} blancos con error de posicion inestable.')
    tirador.descartar_blancos()
    ...
    # Prueba de prediccion de buen desempeño (Posicion inestable)
    print("\nPrueba caso E")
    blancoE = blancoE.reshape(1, 28 * 24)
    print(f'Prediccion blanco con error de posicion inestable: {clf.predict(blancoE)} -> debe ser 1')
    ...
    print(f'Prediccion blanco con error de respiracion: {clf.predict(blancoD)} -> debe ser 0')
